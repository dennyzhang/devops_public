...
## --
## Created : <2016-10-04>
## Updated: Time-stamp: <2016-10-05 09:06:27>
##-------------------------------------------------------------------
...
def haproxy_stats_metric(stat_output, timestamp):
    ...
    for field in 'hrsp_2xx,hrsp_4xx,hrsp_5xx'.split(','):
        print "%s %s %s %s" % (timestamp, field, 'HTTPCode', haproxy_dict[field])

    for field in 'scur,smax'.split(','):
        print "%s %s %s %s" % (timestamp, field, 'SessionThread', haproxy_dict[field])
